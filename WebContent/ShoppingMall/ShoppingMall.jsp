...
<%
	...
	if (request.getAttribute("ProductMap") == null) {
		...
		ProductMapAll = service.getAllProductMapBy_Sts_On();
...
<%
	...
	Map<String, ProductVO> ProductMap = service.findBy_Cate_Sts_OnProduct(it_cate);
			...
			<div class="row">
				<div class="col-xs-12 col-sm-3">
					<div class="list-group MB20">
						<a class="list-group-item list-group-item-success">會員商店</a> <a
							href="<%=request.getContextPath()%>/ShoppingMall/ShoppingMall.jsp"
							class="list-group-item ${(ProductMapAll!=null)?'active':''} ">所有商品</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=snack&it_cate=snack"
							class="list-group-item ${(it_cateActive=='snack')?'active':''}">零食點心區</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=sport&it_cate=sport"
							class="list-group-item ${(it_cateActive=='sport')?'active':''}">休閒運動區</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=tripleC&it_cate=tripleC"
							class="list-group-item ${(it_cateActive=='tripleC')?'active':''}">3C周邊區</a>
					</div>
					<div class="list-group MB20">
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=lifeLiving&it_cate=lifeLiving"
							class="list-group-item ${(it_cateActive=='lifeLiving')?'active':''}">居家生活區</a>


						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=workOut&it_cate=workOut"
							class="list-group-item ${(it_cateActive=='workOut')?'active':''}">運動健身區</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=Game&it_cate=Game"
							class="list-group-item ${(it_cateActive=='Game')?'active':''}">電玩遊戲區</a>
					</div>
					<div class="list-group MB20">
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=furniture&it_cate=furniture"
							class="list-group-item ${(it_cateActive=='furniture')?'active':''}">傢俱寢飾區</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=Women_Clothing&it_cate=Women_Clothing"
							class="list-group-item ${(it_cateActive=='Women_Clothing')?'active':''}">女性服飾區</a>
						<a
							href="<%=request.getContextPath()%>/ProductCategoryController?type=toyForJoy&it_cate=toyForJoy"
				...
				<%
					if (pageContext.getAttribute("ProductMapAll") != null
							&& ((HashMap) pageContext.getAttribute("ProductMapAll")).size() > 0 &&request.getAttribute("ProductSearchMap")==null) {						
				%>
				...
				<div class="col-xs-12 col-sm-9">
					...
					<div class="col-xs-12 col-sm-12">
						<a
							href="${pageContext.request.contextPath}/ShoppingMall/ShoppingMallForOfficial.jsp">官方商店</a>
						<form action="${pageContext.request.contextPath}/SerachProduct"
							...
							<div class="form-group">
								<input type="text" name="It_name" class="form-control"
									...
									type="hidden" name="action" value="SerachFromAll">
					...
					<c:forEach var="ProductMapAll2" items="${ProductMapAll}"
						varStatus="ProductMapAll2status" begin="<%=pageIndex%>"
							...
							<div class="row">
								<div class="col-xs-12 col-sm-4">
									...
									<div class="pic pic-relative">
										<a
											href="${pageContext.request.contextPath}/ShoppingMall/ProductDetail.jsp?it_id=${ProductMapAll2.value.it_id}&st_id=${ProductMapAll2.value.st_id}">
											<img
												...
												<div class="row">
													<form
														...
														<input type=image
															...
															name="action" value="addFavoriteProduct"> <input
				...
				<form action="${pageContext.request.contextPath}/SerachProduct"
					...
					<div class="form-group">
						<input type="text" name="It_name" class="form-control"
							...
							name="action" value="SerachFromAll">
						<button type="submit" class="btn btn-default btn-go ">搜尋</button>
					</div>
				</form>
			</div>
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-xs-12 col-sm-4">
							...
							<div class="pic pic-relative">
								<a
									href="${pageContext.request.contextPath}/ShoppingMall/ProductDetail.jsp?it_id=${ProductSearch.value.it_id}&st_id=${ProductSearch.value.st_id}">
									<img
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-xs-12 col-sm-4">
							...
							<div class="pic pic-relative">
								<a
									href="${pageContext.request.contextPath}/ShoppingMall/ProductDetail.jsp?it_id=${ProductMap2.value.it_id}&st_id=${ProductMap2.value.st_id}">
									<img
										...
										<div class="row">
											<form
												...
												<input type=image
													src="${pageContext.request.contextPath }/img/heart2.png"
													class="minjpg2"> <input type="hidden" name="action"
													value="addFavoriteProductFromCategory"> <input
													type="hidden" name="it_id"
