...
<script>
		$("#cselected").change(function() {
			...
			$.ajax({
				...
				cache: false,
				type : "POST",
				data : productappealVO,
				async: false,
				success : function(data) {
					console.log(data);		
	 				var re = JSON.parse(data);
	 				var getData = re.paLVO;
					...
					$.each(getData, function(index,getData){	
						...
						var tt=$("#tbody");
						tt.append(
						"<tr class='b' style='height:30px'><td><button class='btn btn-primary btn-lg' id='edbutton' value='"+getData.it_apl_id+"' click>編輯</button></td><td>" 
						+getData.pa_date+ "</td><td>"		
						+getData.it_apl_id+ "</td><td>" 
						+getData.apl_sts+ "</td><td>"
						+getData.emp_no+ "</td><td><div class=one_line>" 
						+getData.it_apl_cnt+ "</td></div><td><div class=one_line>"
						+getData.it_rapl+ "</td></div><td <td>" 
						+getData.it_rdate+ "</td></tr>"
						);	
			...
			success : function(data) {
				...
				console.log(data);		
 				...
 				var element = $("<div class='well'>");
		            ...
		            var a2 = $("<td width='180'>商品檢舉編號</td><td width='180'>商品編號</td><td width='180'>檢舉會員編號</td><td width='180'>案件狀態</td><td width='180'>檢舉時間</td>");    
		            a1r.append(a2);

		            var a3r = $("<tr class='table-content'  align='center'>");
		            var a4 = $("<td id='it_id' >"+getData.it_apl_id+"</td><td>"+getData.it_id+"</td><td>"+getData.mem_id+"</td><td>"+getData.apl_sts+"</td><td>"+getData.pa_date+"</td>");
		            a3r.append(a4);
		            ...
		            var a8r = $("<tr class='table-content'  align='center'>");
		       		var a9 = $("<td colspan='4'>"+getData.it_apl_cnt+"</td>");
		       		a8r.append(a9);
		            var $td = $("<td>");
                    ...
                    var a16r = $("<tr align='center'><td colspan='4' class='table-title'>回覆內容</td><td><a class='geto' href='${pageContext.request.contextPath}/productappeal.do?it_apl_id="+getData.it_apl_id+"&action=getOrigin' >查看原始文件</a></td></tr>");
		                            
                    
                    var a17r = $("<tr align='center'>");
		            var a18 = $("<td colspan='4' class='table-content'><textarea id='it_rapl' class='form-control' rows='2' id='comment' name='art_rapl' required></textarea></td>");
	...
	$(document).on("click","#btnsubmit",function()
		{
			...
			var apl_sts;
			var it_apl_id;
			var it_rapl = $("#it_rapl").val();
			if(it_rapl =="" || it_rapl.trim().length ==0){
				alert("回復檢舉不得為空白");
				return false
			}
