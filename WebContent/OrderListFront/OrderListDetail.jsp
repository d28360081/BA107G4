			...
			<c:forEach var="product" items="${productMap}">
				<c:if
					...
					<div class="modal-footer">
						...
						<c:if test="${storeOrder.rec_dlv_sts eq '已出貨'}">
							<form
								action="${pageContext.request.contextPath}/StoreReceiptDetailController"
								method="post"
								onsubmit="return ChangeStoreOrder_sts_finished(this) ">
								<div class="btn-group btn-group-lg">
									<input type="submit" value="確認貨款已到達" class="btn btn-info">
									<input type="hidden" name="action"
										value="ChangeStoreOrder_sts_finished"> <input
										type="hidden" name="rec_id"
										value="${storeReceiptDetail.rec_id}">
								</div>
							</form>
						</c:if>
						...
						<c:if
							...
							<form
								...
								<div class="btn-group btn-group-lg">
									...
									<input type="hidden" name="action"
										value="CancelOrderFromOrderList"> <input type="hidden"
