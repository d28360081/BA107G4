...
public class ComNofJndi implements ComNofInterface{
	...
	public void update(ComNofVO comnofvo) throws RuntimeException{
		...
		c=ds.getConnection();
		c.setAutoCommit(false);
		PreparedStatement prst =c.prepareStatement("update notification set mem_id=?,NOF_TIT=?,nof_cnt=?,nof_sts=? where nof_id=?");
		prst.setString(1, comnofvo.getMem_id());
		prst.setString(2, comnofvo.getNof_tit());
		prst.setString(3, comnofvo.getNof_cnt());
		prst.setString(4,comnofvo.getNof_sts());
		prst.setString(5, comnofvo.getNof_id());
		prst.executeUpdate();
	...
	public List<ComNofVO> selectAllUnread(String mem_id) {
		   Connection c=null;
					...
					c=ds.getConnection();
					c.setAutoCommit(false);
					PreparedStatement prst =c.prepareStatement("select * from NOTIFICATION where mem_id =? and nof_sts='未讀' and com_id is null");
					prst.setString(1, mem_id);
					ResultSet rs=prst.executeQuery();
					List<ComNofVO> list=new ArrayList<ComNofVO>();
				    while(rs.next()){
